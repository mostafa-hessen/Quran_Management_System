  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2, 1fr)', lg: 'repeat(4, 1fr)' }, gap: 2 }}>
        <StatCard title="الطلاب" value={stats?.total_students || 0} icon={<PeopleAltRounded fontSize="large" />} color="#047857" loading={isLoading} />
        <StatCard title="المعلمون" value={stats?.total_teachers || 0} icon={<MenuBookRounded fontSize="large" />} color="#1d4ed8" loading={isLoading} />
        <StatCard title="الحلقات" value={stats?.total_halaqat || 0} icon={<HexagonRounded fontSize="large" />} color="#b45309" loading={isLoading} />
        <StatCard title="متأخرات مالية" value={stats?.overdue_count || 0} icon={<WarningRounded fontSize="large" />} color="#b91c1c" loading={isLoading} />
      </Box>
          ...
          {stats?.overdue_count > 0 && (
            <Box sx={{ display: 'flex', alignItems: 'center', p: 2, bgcolor: '#fef2f2', border: '1px solid #fee2e2', borderRadius: 3, gap: 2 }}>
              ...
              <Chip 
                label="عرض" 
                size="small" 
                sx={{ bgcolor: '#fee2e2', color: '#b91c1c', fontWeight: 'bold' }} 
                clickable 
                component="a"
                href="/payments"
              />
