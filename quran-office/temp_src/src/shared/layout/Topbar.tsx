...
const Topbar: React.FC<TopbarProps> = ({ onMenuClick }) => {
  ...
  const getPageTitle = (path: string) => {
    switch (path) {
      ...
      case '/teachers': return 'المعلمون المعلمات';
      case '/students': return 'الطلاب والطالبات';
      case '/staff': return 'إدارة الموظفين';
      case '/enrollments': return 'التسجيل والطلبات';
      default: return 'علمه البيان';
    }
  };
    ...
    >
      <Toolbar sx={{ justifyContent: "space-between", px: { xs: 2, md: 4 } }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 3 }}>
          ...
          <Box>
            <Typography variant="h6" sx={{ fontWeight: 800, fontSize: "1.25rem", color: "#1F2937" }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 1, md: 2 } }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, ml: 1 }}>
              <Box sx={{ textAlign: 'left', display: { xs: 'none', sm: 'block' } }}>
                  <Typography variant="body2" sx={{ fontWeight: 700, lineHeight: 1, textAlign: 'left' }}>
