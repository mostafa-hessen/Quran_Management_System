-- ================================================================
-- MIGRATION: تنظيم إدارة الموظفين وفصل بيانات المعلمين التفصيلية
-- ================================================================

-- ١. تأمين جدول profiles من الحقول المفقودة للأدمن والسكرتير
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS phone TEXT,
ADD COLUMN IF NOT EXISTS hire_date DATE;

-- ٢. تحديث دالة admin_setup_staff لفك التداخل
CREATE OR REPLACE FUNCTION public.admin_setup_staff(
    ...
    p_phones       JSONB    DEFAULT '[]'::JSONB, -- للهواتف المتعددة (للمعلمين فقط)
...
BEGIN
    ...
    IF p_role = 'teacher' THEN
        ...
        -- إضافة الهواتف المتعددة للمعلم فقط
...
-- ٣. تحديث دالة admin_update_user
...
BEGIN
    ...
    -- تحديث الاسم الكامل في البروفايل
    SELECT TRIM(CONCAT_WS(' ', COALESCE(p_first_name, ''), COALESCE(p_father_name, ''), COALESCE(p_grandfather_name, ''), COALESCE(p_family_name, ''))) INTO v_full_name;
    IF v_full_name <> '' THEN
