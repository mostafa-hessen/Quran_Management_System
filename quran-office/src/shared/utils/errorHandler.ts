...
export const handleAppError = (error: any): AppError => {
  ...
  if (message.includes('unique constraint') || message.includes('already exists') || message.includes('already been registered')) {
    return { message: 'البريد الإلكتروني أو هذه البيانات مسجلة مسبقاً لمستخدم آخر.', code: 'duplicate', originalError: error };
  }
