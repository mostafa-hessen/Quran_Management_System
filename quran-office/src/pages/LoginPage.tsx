...
import { Box, Typography, Button, Paper, TextField, InputAdornment, IconButton, CircularProgress, Alert } from '@mui/material';
import { 
...
const LoginPage: React.FC = () => {
  ...
  const errorMsg = (signInMutation.error?.message || signUpMutation.error?.message) || '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      signUpMutation.mutate({ email, password, role, fullName }, {
        onSuccess: () => {
          alert('تم إنشاء الحساب بنجاح! إذا كانت هناك رسالة تفعيل يرجى مراجعة بريدك.');
          setIsLogin(true);
        }
      });
    }
    ...
    >
      <Box sx={{ width: '100%', maxWidth: 450 }}>
        ...
        >
          ...
          {!isLogin && (
            <Box>
              ...
              <Box sx={{ display: 'flex', gap: 1 }}>
                {[
                  { id: 'teacher', label: 'معلم', icon: <MenuBookRounded fontSize="small" /> },
                  { id: 'supervisor', label: 'سكرتيرة', icon: <AssignmentIndRounded fontSize="small" /> },
                  { id: 'admin', label: 'إدارة', icon: <AdminPanelSettingsRounded fontSize="small" /> }
                ].map((item) => (
                  <Button
                    key={item.id}
          ...
          >
            {loading ? <CircularProgress size={24} color="inherit" /> : (isLogin ? 'دخول' : 'تسجيل')}
          </Button>
